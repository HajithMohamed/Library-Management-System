...
// ==========================================
        ...
        // ==========================================
// STEP 11: Create e_resources table
// ==========================================
        $currentStep++;
        echo "<h2>Step {$currentStep}/{$totalSteps}: Creating e_resources table...</h2>";

        try {
            $tableExists = $conn->query("SHOW TABLES LIKE 'e_resources'")->num_rows > 0;

            if (!$tableExists) {
                $sql = "CREATE TABLE e_resources (
            ...
            INDEX idx_status (status)
                ...
                $conn->query($sql);
                echo "<div class='success'>✓ e_resources table created successfully</div>";
            } else {
                echo "<div class='info'>ℹ e_resources table already exists. Checking column types...</div>";
                // Check if uploadedBy is INT and needs to be VARCHAR
                $result = $conn->query("SHOW COLUMNS FROM e_resources LIKE 'uploadedBy'");
                if ($result && $row = $result->fetch_assoc()) {
                    if (stripos($row['Type'], 'int') !== false) {
                        echo "<div class='warning'>⚠ Column 'uploadedBy' is INT but should be VARCHAR. Altering table...</div>";
                        $conn->query("ALTER TABLE e_resources MODIFY uploadedBy VARCHAR(255) NOT NULL");
                        echo "<div class='success'>✓ Column 'uploadedBy' altered to VARCHAR(255) successfully</div>";
                    } else {
                        echo "<div class='success'>✓ Column 'uploadedBy' type is correct (" . $row['Type'] . ")</div>";
                    }
                }
            }
        } catch (Exception $e) {
        ...
        // ==========================================
// STEP 12: Create user_eresources table
// ==========================================
        $currentStep++;
        echo "<h2>Step {$currentStep}/{$totalSteps}: Creating user_eresources table...</h2>";

        try {
            $tableExists = $conn->query("SHOW TABLES LIKE 'user_eresources'")->num_rows > 0;

            if (!$tableExists) {
                $sql = "CREATE TABLE user_eresources (
            id INT AUTO_INCREMENT PRIMARY KEY,
            ...
            INDEX idx_resource (resource_id)
                ...
                $conn->query($sql);
                echo "<div class='success'>✓ user_eresources table created successfully</div>";
            } else {
                echo "<div class='info'>ℹ user_eresources table already exists</div>";
            }
        } catch (Exception $e) {
            echo "<div class='error'>✗ Error: {$e->getMessage()}</div>";
        }
