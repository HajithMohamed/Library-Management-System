...
class AdminController
{
  ...
  public function roles()
  {
    ...
    // Handle Role Management (Add/Edit logic would go here)
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      // Placeholder for handling role updates
    ...
    if ($roleResult) {
      while ($role = $roleResult->fetch_assoc()) {
        ...
        $pidResult = $conn->query("SELECT p.name, p.slug FROM permissions p JOIN permission_role pr ON p.id = pr.permission_id WHERE pr.role_id = {$role['id']}");
        $role['permissions'] = [];
        while ($perm = $pidResult->fetch_assoc()) {
          $role['permissions'][] = $perm;
        }
        $roles[] = $role;
      }
    } else {
      // Fallback if tables don't exist yet (migration failure simulation)
      $roles = [
        ['name' => 'Admin', 'slug' => 'admin', 'description' => 'Administrator', 'permissions' => [['name' => 'All Access', 'slug' => '*']]],
        ['name' => 'Student', 'slug' => 'student', 'description' => 'Student', 'permissions' => [['name' => 'Borrow Books', 'slug' => 'books.read']]]
      ];
    }

    // Fetch all available permissions
    $allPermissions = [];
    $permResult = $conn->query("SELECT * FROM permissions");
    if ($permResult) {
    ...
    include APP_ROOT . '/views/admin/roles.php';
  }
