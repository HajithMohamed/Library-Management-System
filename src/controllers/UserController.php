...
class UserController extends BaseController
{
    ...
    public function dashboard()
    {
        ...
        if (!$userId) {
            \Helpers\NotificationHelper::error('User ID not found. Please login again.');
            $this->redirect('login');
            return;
        }
    ...
    public function verifyPasswordOtp()
    {
        ...
        $stmt = $this->db->prepare("UPDATE users SET password = ?, password_changed = 1, password_changed_at = NOW(), updatedAt = NOW() WHERE userId = ?");
